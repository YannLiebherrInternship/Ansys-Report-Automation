...
Trois méthodes sont disponibles. Dans les trois cas, l'interface obtenue une fois l'application ouverte (voir §6) est strictement identique — seuls le mode de déploiement, le nom sous lequel l'outil apparaît, et le mode de lancement changent.

| | Méthode 1 — Extension `.wbex` | Méthode 2 — Extension ACT manuelle | Méthode 3 — Bouton manuel (console) |
|---|---|---|---|
| Dossier source (ce dépôt) | `Reports/Liebherr Report Generator.xml` + `Reports/Liebherr Report Generator/` | `Reports/Liebherr Report Generator.xml` + `Reports/Liebherr Report Generator/` (identique à la Méthode 1) | `Report Generator/` (dossier séparé, à la racine du dépôt) |
| Installation | Une seule fois (Extension Manager), valable pour tous les projets | Une seule fois (dossier "extension" + Options), valable pour tous les projets | Copie manuelle d'un dossier, projet par projet |
| Chemins de fichiers | Gérés automatiquement, l'extension détecte seule le projet Workbench hôte | Identique à la Méthode 1 | Calculés au premier lancement à partir du dossier `Report Generator` créé à la main dans le projet |
| Accès | Onglet dédié dans le ruban ACT, intégré à l'extension | Identique à la Méthode 1 | Bouton "promu" dans l'onglet Automation, persistant entre les sessions d'un même projet |
| Nettoyage des fichiers | Onglet "Files" de l'extension — manuel, vérification finale requise | Identique à la Méthode 1 | Onglet "Files" de l'application (voir §6) |
| Désinstallation | Automatique : l'extension supprime tous les fichiers qui lui sont associés | Décocher/retirer l'extension dans l'Extension Manager | Suppression manuelle du dossier `Report Generator` |

### 2.1 Méthode 1 — Extension Ansys (`.wbex`)
...
### 2.2 Méthode 2 — Extension ACT installée manuellement (sans `.wbex`)

Permet d'installer la même extension ACT que la Méthode 1, sans passer par un fichier `.wbex` déjà construit — en repartant directement des fichiers sources du dépôt. Utile pour tester une modification locale du code avant de la publier sous forme de `.wbex`, ou si aucun fichier `.wbex` n'est disponible.

1. Repérer dans ce dépôt le fichier `Reports/Liebherr Report Generator.xml` et le dossier `Reports/Liebherr Report Generator/` du même nom (le manifeste et son payload — voir §3).
2. Créer un dossier nommé exactement `extension` sur la machine (emplacement testé : à côté d'un dossier `user_files` d'un projet Ansys — l'emplacement exact importe peu, tant qu'il s'agit d'un dossier stable sur le poste, puisqu'il est enregistré une seule fois comme dossier d'extensions supplémentaire à l'étape suivante).
3. Dans Ansys Workbench : **Tools** → **Options** → **Extensions**, puis coller le chemin complet du dossier `extension` créé à l'étape précédente (sans guillemets).
4. Aller dans ce dossier `extension` et y déposer le dossier `Liebherr Report Generator/` **et** le fichier `Liebherr Report Generator.xml` du même nom (les deux, côte à côte, comme dans ce dépôt).
5. Toujours dans Workbench, ouvrir l'**Extension Manager**, cocher l'extension "Liebherr Report Generator", recharger, puis ouvrir Mechanical pour l'utiliser — l'onglet apparaît comme pour la Méthode 1.
6. **Optionnel** : à partir de cette installation manuelle, il est possible de générer son propre fichier `.wbex` (pour le redistribuer ensuite comme à la Méthode 1) en allant dans la page de démarrage ACT (**ACT Start Page**) et en cliquant sur **"Build"** dans les paramètres de l'extension chargée.

### 2.3 Méthode 3 — Bouton manuel dans la console de scripting

Contrairement aux deux méthodes précédentes, **ce n'est pas une extension ACT** : aucun fichier `.xml`, aucun manifeste, aucun `oninit`/`onclick` — le script `AnsysReportGenerator_WPF.py` s'exécute directement du début à la fin quand on le charge dans la console de scripting, exactement comme n'importe quel autre script Mechanical. C'est la méthode la plus simple à mettre en place pour tester rapidement une modification du code sans passer par ACT.

> Le dossier `Report Generator/` à la racine de ce dépôt est le point de départ de cette méthode : il est volontairement **séparé** de `Reports/Liebherr Report Generator/` (Méthodes 1 et 2, qui sont une vraie extension ACT). Il contient une copie du même code applicatif, mais sans manifeste et sans le point d'entrée `init`/`HighFiveOut` d'ACT — le script se lance directement à l'exécution, comme le faisait le tout premier script de bureau de ce projet avant sa conversion en extension.
...
3. Copier à plat dans ce dossier, sans sous-dossier, tout le contenu du dossier `Report Generator/` de ce dépôt :
...
Ce dépôt contient deux payloads distincts et volontairement séparés (voir §2) :

| Dossier | Rôle |
|---|---|
| `Reports/Liebherr Report Generator.xml` + `Reports/Liebherr Report Generator/` | Extension ACT complète (manifeste + payload) — Méthodes 1 et 2 (§2.1, §2.2) |
| `Report Generator/` | Même application, sans habillage ACT (pas de manifeste, script à exécution directe) — Méthode 3 (§2.3) |

Le reste de cette documentation (§4 et suivantes) décrit le code applicatif commun aux trois méthodes ; le dossier `Report Generator/` correspond au contenu à déployer pour la Méthode 3 (§2.3).

Détail du contenu de `Report Generator/` (Méthode 3) :
