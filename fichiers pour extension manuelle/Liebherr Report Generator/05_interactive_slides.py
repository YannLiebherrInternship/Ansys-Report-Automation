...
# Column 2 (ViewOrientationType) mapped from the axis actually perpendicular to the screen for
# each named .NET orientation, confirmed visually in Mechanical:
#   Back = Z-, Front = Z+, Left = X-, Right = X+, Top = Y+, Bottom = Y-, Iso = Iso
# e.g. selecting "Z-" must apply ViewOrientationType.Back, since Back is the viewport where Z- is
# the axis pointing out of the screen.
...
def create_basic_views():
    ...
    # No Transaction(True) here (removed): it deferred the viewport refresh until all 7 views
    # were created, on the assumption that CreateView() only reads the in-memory camera state,
    # not an actually-rendered frame. That assumption was never confirmed and is the suspected
    # cause of views being captured with the wrong orientation (e.g. "Z-" baking in a state that
    # visually reads as "Y-"). Each orientation is now set, redrawn, and captured one at a time.
    created = []
...
# Ordered (value, label) pairs for the "Image format" combo box (see _build_image_format_fields
# in AnsysReportGenerator_WPF.py) - standard formats first, then the HQ (double resolution) ones.
IMAGE_FORMAT_OPTIONS = [
    (IMAGE_FORMAT_SQUARE, "Square (1920x1920)"),
    (IMAGE_FORMAT_RECTANGULAR, "Horizontal (1920x1080)"),
    (IMAGE_FORMAT_SQUARE_HQ, "Square HQ (3840x3840)"),
    (IMAGE_FORMAT_RECTANGULAR_HQ, "Horizontal HQ (3840x2160)"),
]
IMAGE_FORMAT_LABELS = dict(IMAGE_FORMAT_OPTIONS)


def image_format_label(value):
    """
    Does: finds the label shown for an image_format value (row summary chip, and the "Image
    format" combo box in the settings panel, see _build_image_format_fields).
    Depends on: IMAGE_FORMAT_LABELS.
    Returns: str, the matching label (the one for DEFAULT_IMAGE_FORMAT if value is unknown).
    """
    return IMAGE_FORMAT_LABELS.get(value, IMAGE_FORMAT_LABELS[DEFAULT_IMAGE_FORMAT])


def image_format_from_label(label):
    """
    Does: finds the image_format value associated with a label from IMAGE_FORMAT_OPTIONS.
    Depends on: IMAGE_FORMAT_OPTIONS.
    Returns: str, the matching value (DEFAULT_IMAGE_FORMAT if the label is unknown).
    """
    for value, option_label in IMAGE_FORMAT_OPTIONS:
...
class SlideRowConfig(object):
    ...
    def __init__(self, obj, analysis=None):
        ...
        self.view_name = None
        self.image_format = DEFAULT_IMAGE_FORMAT  # "square" (1920x1920) or "rectangular" (1920x1080), see IMAGE_FORMAT_DIMENSIONS
...
def build_row_display_name(row_config):
    ...
    if row_config.image_format and row_config.image_format != DEFAULT_IMAGE_FORMAT:
        parts.append("format=" + image_format_label(row_config.image_format))
...
def export_result_image_for_step(result_obj, step_number, image_format=None):
    """
    ...
    return export_current_view_image("{}_step{}".format(result_obj.Name, step_number), image_format)
...
def capture_multi_result_cell_image(cfg, views, section_planes, section_labels):
    ...
    try:
        ...
        image_path = export_solution_image(obj, cfg.image_format)
    except Exception as e:
...
def build_step_based_result_slides(report, cfg, obj, subtitle, analysis):
    ...
    try:
        if template:
            image_paths = [export_result_image_for_step(obj, step, cfg.image_format) for step in steps]
        ...
        for step in steps:
            img_path = None
            try:
                img_path = export_result_image_for_step(obj, step, cfg.image_format)
            except Exception as e:
...
def build_single_bc_slide(report, cfg, views, section_planes, section_labels):
    ...
    try:
        ...
        img_path = export_object_image(bc, bc.Name, cfg.image_format)
        disable_all_section_planes(section_planes)
        reset_scale_factor()
        reset_contour_view()
        reset_legend_orientation()
...
def build_single_bp_slide(report, cfg, views, section_planes, section_labels):
    ...
    try:
        ...
        img_path = export_object_image(bp, bp.Name, cfg.image_format)
        disable_all_section_planes(section_planes)
        reset_scale_factor()
        reset_contour_view()
        reset_legend_orientation()
...
def build_single_result_slide(report, cfg, subtitle, views, section_planes, section_labels, analysis):
    ...
    try:
        ...
        if cfg.selected_steps:
            ...
            img_path = None
            try:
                img_path = export_object_image(obj, obj.Name, cfg.image_format)
            except Exception as e:
...
def export_geometry_part_image(body, all_bodies, context_opacity_percent, image_format=None):
    """
    ...
    image_path = export_current_view_image("Geometry_" + safe_file_name(body.Name), image_format)
...
def build_single_geometry_part_slide(report, cfg, all_bodies, views, section_planes, section_labels):
    ...
    try:
        apply_view_if_exists(cfg.view_name, views)
        apply_section_plane(section_planes, section_labels, cfg.section_name)
        img_path = export_geometry_part_image(body, all_bodies, cfg.context_opacity_percent, cfg.image_format)
...
def export_body_mesh_image(body, all_bodies, image_name, image_format=None):
    """
    ...
    image_path = export_current_view_image(image_name, image_format)
...
def build_mesh_slide(report, use_full_table, image_format=None):
    """
    ...
    Returns: nothing (side effect: adds a slide to the report).
    """
    img_path = export_mesh_image(image_format)
    if use_full_table:
...
def build_analysis_context_slides(report, row_configs, views):
    ...
    for cfg in row_configs:
        apply_view_if_exists(cfg.view_name, views)
        create_analysis_parameters_slide(report, cfg.obj, cfg.image_format)
