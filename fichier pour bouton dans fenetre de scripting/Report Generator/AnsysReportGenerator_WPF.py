# AnsysReportGenerator_WPF.py : WPF entry point for the report generation application. Meant to be
# loaded directly in Mechanical's scripting console (Automation tab) - not an ACT extension, see
# README.md "Manual installation (scripting console)". Loads modules 00_constants.py to
# 05_interactive_slides.py (same folder as this script) via execfile(), then builds the window
# from AnsysReportGenerator_WPF.xaml and shows it - running this file (or a button promoted from
# it) is the only thing needed, no callback/manifest involved.
...
# This whole block lives inside a function, called once at the bottom of this file (SECTION 8),
# rather than at module level: it needs ExtAPI.DataModel.Project (see PROJECT_DIR below), which
# only exists once a project is open in Mechanical - by the time this script is run from the
# scripting console (or a button promoted from it), that is already the case. execfile() runs
# each file in this script's global namespace (explicit 2nd argument = globals()), as if its
# content had been copy-pasted into the console right after the others.


def _prepare_environment():
    """
    Does: locates PROJECT_DIR next to the current project's user_files, loads modules 00 -> 05 and the default file paths.
    Depends on: ExtAPI.DataModel.Project.ProjectDirectory, execfile(..., globals()).
    ...
    # PROJECT_DIR = "Report Generator" folder created by hand next to "user_files", inside the
    # "<Project>_files" folder of the Ansys project currently open in Mechanical (see README.md,
    # "Manual installation (scripting console)") - one copy per project, unlike the ACT extension
    # flavor of this same project (see "Reports/Liebherr Report Generator"), which is installed
    # once and shared across every project.
    #
    # Located via ExtAPI.DataModel.Project.ProjectDirectory rather than __file__: this script is
    # meant to be run directly from the scripting console (or a button promoted from it), where
    # __file__/os.getcwd() are not reliably set to this script's real path - unlike an ACT
    # extension, where __file__ is set by ACT itself from the manifest's <script src>.
    PROJECT_DIR = os.path.join(ExtAPI.DataModel.Project.ProjectDirectory, "Report Generator")

    if not os.path.isfile(os.path.join(PROJECT_DIR, "00_constants.py")):
        raise IOError(
            "Missing modules next to AnsysReportGenerator_WPF.py: {}. Check that a 'Report "
            "Generator' folder exists next to this project's user_files, and that 00_constants.py "
            "-> 05_interactive_slides.py, AnsysReportGenerator_WPF.xaml and the PowerPoint "
            "template are all present in it (see README.md).".format(PROJECT_DIR)
        )
    ...
    FILE_PATH_SETTINGS = [
        ("TEMPLATE_PATH", "txtPathTemplate", "btnBrowseTemplate", "file", "btnShowTemplate"),
        ("IMAGE_EXPORT_FOLDER", "txtPathImages", "btnBrowseImages", "folder", "btnShowImages"),
        ("CSV_EXPORT_FOLDER", "txtPathCsv", "btnBrowseCsv", "folder", "btnShowCsv"),
        ("EXPORT_3D_FOLDER", "txtPathExport3D", "btnBrowseExport3D", "folder", "btnShowExport3D"),
        ("LEGEND_FOLDER", "txtPathLegends", "btnBrowseLegends", "folder", "btnShowLegends"),
        ("REPORT_OUTPUT_FOLDER", "txtPathReports", "btnBrowseReports", "folder", "btnShowReports"),
    ]

    _DEFAULT_FILE_PATHS = dict((name, globals()[name]) for name, _, _, _, _ in FILE_PATH_SETTINGS)
...
# Same fade principle as ITEM_LIST_FADE_HEIGHT, applied to the configuration side panel's
# ScrollViewer (borderConfigPanel/scrollConfigPanel): taller than the row lists' fade because
# it has to blend into the fixed Apply/Cancel footer overlaid on top of it (panelConfigPanelFooter,
# see ReportGeneratorApp._open_config_panel), itself taller than a simple fade strip.
CONFIG_PANEL_FOOTER_FADE_HEIGHT = 60
...
def _build_image_format_fields(target, root, row_config):
    """
    Does: builds the "Image format" field (a combo box listing every entry of IMAGE_FORMAT_OPTIONS)
    and adds it to root, right below the view field in every settings panel that has one. Shared by
    _build_row_config_fields/_build_geometry_part_fields/_build_mesh_part_fields.
    Depends on: _make_field_label, IMAGE_FORMAT_OPTIONS/image_format_label (05_interactive_slides.py).
    Returns: nothing (sets on target: cmb_image_format).
    """
    root.Children.Add(_make_field_label("Image format:"))

    target.cmb_image_format = ComboBox()
    target.cmb_image_format.Margin = Thickness(0, 4, 0, 12)
    for value, label in IMAGE_FORMAT_OPTIONS:
        target.cmb_image_format.Items.Add(label)
    target.cmb_image_format.SelectedItem = image_format_label(row_config.image_format)
    root.Children.Add(target.cmb_image_format)


def _apply_image_format_fields(target, row_config):
    """
    Does: reads the "Image format" combo box from target and applies the selection to row_config.
    Depends on: target.cmb_image_format, image_format_from_label (05_interactive_slides.py).
    Returns: nothing (side effect on row_config.image_format).
    """
    row_config.image_format = image_format_from_label(unicode(target.cmb_image_format.SelectedItem))
...
def _build_row_config_fields(target, root, row_config, views, section_plane_labels, legend_names):
    ...
    Depends on: _make_field_label, _build_image_format_fields, get_result_display_unit, CONTOUR_VIEW_OPTIONS/LEGEND_ORIENTATION_OPTIONS/SCOPING_DISPLAY_OPTIONS.
    Returns: nothing (sets on target: cmb_view/cmb_image_format/cmb_section/cmb_legend/cmb_contour_view/cmb_legend_orientation/cmb_scoping_display/txt_scale).
    """
    ...
    _build_image_format_fields(target, root, row_config)
...
def _apply_row_config_fields(target, row_config):
    """
    Does: reads the common fields (view/format/section/legend/appearance/scoping/scale factor) from
    target and applies them to row_config. Shared by the global side panel (_on_config_panel_apply)
    and by the inline row panel of the "Combined slide" tab (see _build_row_config_fields).
    Depends on: target.cmb_view/cmb_image_format/cmb_section/cmb_legend/cmb_contour_view/
        cmb_legend_orientation/cmb_scoping_display/cmb_deformation_scale_mode/txt_scale, _apply_image_format_fields.
    ...
    _apply_image_format_fields(target, row_config)
...
def _build_geometry_part_fields(target, root, row_config, views, section_plane_labels):
    """
    Does: builds the view/format/section/context opacity fields for an isolated part (geometry) and adds them to root.
    Depends on: _make_field_label, _build_image_format_fields.
    Returns: nothing (sets on target: cmb_view/cmb_image_format/cmb_section/slider_opacity/lbl_opacity_value).
    """
    root.Children.Add(_make_field_label("View (View Manager):"))
    target.cmb_view = ComboBox()
    target.cmb_view.Margin = Thickness(0, 4, 0, 12)
...
def _apply_geometry_part_fields(target, row_config):
    """
    Does: reads the view/format/section/opacity fields from target and applies them to row_config.
    Depends on: target.cmb_view/cmb_image_format/cmb_section/slider_opacity, _apply_image_format_fields.
...
def _build_mesh_part_fields(target, root, row_config, views):
    """
    Does: builds the view and image format fields (View Manager) only and adds them to root - no
    section or opacity (unlike _build_geometry_part_fields): isolation is done by fully hiding
    the other bodies (show_only_body), a context section/opacity would not make sense
    here. Reused as-is for Geometry/Mesh/Analysis context (view+format only there too).
    Depends on: _make_field_label, _build_image_format_fields.
    Returns: nothing (sets on target: cmb_view/cmb_image_format).
    """
    root.Children.Add(_make_field_label("View (View Manager):"))
    target.cmb_view = ComboBox()
    target.cmb_view.Margin = Thickness(0, 4, 0, 12)
...
def _apply_mesh_part_fields(target, row_config):
    """
    Does: reads the view/format fields from target and applies them to row_config.
    Depends on: target.cmb_view/cmb_image_format, _apply_image_format_fields.
    Returns: nothing (side effect on row_config).
    """
    selected_view = unicode(target.cmb_view.SelectedItem)
    row_config.view_name = None if selected_view == NO_VIEW_LABEL else selected_view
...
class ReportGeneratorApp(object):
    ...
    def __init__(self, xaml_path):
        ...
        # "Export 3D" tab: deliberately separate from self._sections/_section_order (own rows
        # over the same objects) so that checking a row here never adds a slide to the report.
        self._export3d_sections = {}
        ...
        self._mr_picker_entries = []      # ordered dicts (header/row) of the "choose a result" list, see _show_multi_result_picker
        self._mr_picker_list_panel = None  # StackPanel holding the "choose a result" rows (reordered in place by search)
        ...
        self._current_report = None    # PPTReportBuilder kept open across "Generate report" clicks (appends instead of reopening), see _get_or_create_report
        ...
        self._find_controls()
        self._attach_list_fade(self.scroll_config_panel, CONFIG_PANEL_FOOTER_FADE_HEIGHT)
        self._refresh_general_slide_status()
        self._build_sections()
        self._build_export3d_sections()
        self._build_multi_result_tab()
        self._wire_contacts_filter()
        self._wire_zone_select_buttons()
        self._wire_export3d_zone_buttons()
    ...
    def _find_controls(self):
        ...
        self.scroll_config_panel = w.FindName("scrollConfigPanel")
        self.panel_config_panel = w.FindName("panelConfigPanel")
        self.panel_config_panel_footer = w.FindName("panelConfigPanelFooter")
        ...
        self.btn_check_all_export3d = w.FindName("btnCheckAllExport3D")
        self.btn_uncheck_all_export3d = w.FindName("btnUncheckAllExport3D")
        self.btn_export_selected_3d = w.FindName("btnExportSelected3D")
        self.lbl_export3d_estimate = w.FindName("lblExport3DEstimate")
    ...
    def _wire_file_paths(self):
        """
        Does: initializes the path TextBoxes with their current value and wires the "..."/"Show in
        folder"/reset buttons.
        Depends on: FILE_PATH_SETTINGS, self._make_path_edit_handler/_make_path_browse_handler/
            _make_path_show_handler/_on_reset_file_paths.
        Returns: nothing (side effect on the Files tab controls).
        """
        self._path_textboxes = {}
        for name, textbox_id, browse_id, kind, show_id in FILE_PATH_SETTINGS:
            textbox = self.window.FindName(textbox_id)
            browse_button = self.window.FindName(browse_id)
            show_button = self.window.FindName(show_id)
            ...
            show_button.Click += self._make_path_show_handler(name, kind)
    ...
    def _make_path_show_handler(self, name, kind):
        """
        Does: closes over name/kind by value to produce the Click handler of the "Show in folder" button of a path row.
        Depends on: self._on_show_path_in_folder, globals()[name].
        Returns: function, the handler(sender, e) to wire on show_button.Click.
        """
        def handler(sender, e):
            """
            Does: shows the row's CURRENT path (read from the global at click time, so it reflects
            any edit made via the TextBox/browse dialog since the row was built).
            Depends on: self._on_show_path_in_folder, name/kind captured by the closure.
            Returns: nothing (side effect: launches explorer.exe).
            """
            self._on_show_path_in_folder(globals()[name], kind)
        return handler
    ...
    def _on_show_path_in_folder(self, path, kind):
        """
        Does: "Show in folder" for a File paths row (kind="file"/"folder") or a Data folder
        cleanup tile (always a folder) - selects the file within its parent (kind="file", via
        self._on_show_in_folder) or opens the folder itself directly (kind="folder": a folder has
        no parent to select it within, unlike a file).
        Depends on: self._on_show_in_folder, ensure_folder_exists (00_constants.py), Process.Start.
        Returns: nothing (side effect: launches explorer.exe, shows a MessageBox on failure).
        """
        if kind == "file":
    ...
    def _build_data_cleanup_tile(self, name, path):
        """
        Does: builds a cleanup tile (folder name, size, file count, "Clear"/"Show in folder" buttons).
        Depends on: get_folder_stats/format_folder_size (00_constants.py), self._make_clear_folder_handler/_make_show_folder_handler, _shared_resources, CARD_NORMAL_BACKGROUND.
        ...
        buttons_row = StackPanel()
        buttons_row.Orientation = Orientation.Horizontal
        ...
        buttons_row.Children.Add(btn_clear)

        btn_show = Button()
        btn_show.Content = "Show in folder"
        btn_show.Style = _shared_resources["SecondaryButton"]
        btn_show.Padding = Thickness(8, 3, 8, 3)
        btn_show.FontSize = 11
        btn_show.Margin = Thickness(6, 0, 0, 0)
        btn_show.Click += self._make_show_folder_handler(path)
        buttons_row.Children.Add(btn_show)

        content.Children.Add(buttons_row)
    ...
    def _make_show_folder_handler(self, path):
        """
        Does: closes over path by value to produce the Click handler of a cleanup tile's "Show in folder" button.
        Depends on: self._on_show_path_in_folder.
        Returns: function, the handler(sender, e) to wire on btn_show.Click.
        """
        def handler(sender, e):
            self._on_show_path_in_folder(path, "folder")
        return handler
    ...
    def _build_export3d_sections(self):
        """
        Does: populates the "Export 3D" tab's 3 selection zones (Contact Tool Results/Results/
        Bolt Tool) from the same object lists as the "Result categories" tab (self._contact_tool_results/
        _results/_bolt_tool_results), as plain checkbox rows (panel_kind=None: no "..." button,
        an .avz capture has no per-row settings). Stored in self._export3d_sections rather than
        self._sections/_section_order: unlike the "Result categories" rows, checking one of these
        must never add a slide to the report (see self._collect_desired_preview_entries).
        Depends on: self._contact_tool_results/_results/_bolt_tool_results, self._build_section, self._wire_search_box, self._attach_list_fade.
        Returns: nothing (side effect: fills self._export3d_sections and the WPF panels).
        """
        section_defs = [
            ("ContactTool3D", "panelContactTool3D", "searchContactTool3D", self._contact_tool_results),
            ("Results3D", "panelResults3D", "searchResults3D", self._results),
            ("BoltTool3D", "panelBoltTool3D", "searchBoltTool3D", self._bolt_tool_results),
        ]

        for name, panel_name, search_name, objects in section_defs:
            panel = self.window.FindName(panel_name)
            search_box = self.window.FindName(search_name)
            self._init_search_placeholder(search_box)
            rows = self._build_section(panel, objects, SlideRowConfig, build_row_display_name,
                                        None, True)

            self._export3d_sections[name] = {"rows": rows, "panel": panel}
            self._wire_search_box(search_box, rows, panel)
            self._attach_list_fade(panel.Parent)

            # Additional handler on top of the one already wired by _build_row (multicast
            # delegate: both run) - keeps the duration estimate in sync with the checkboxes
            # without touching the shared row-building code used by the other tabs.
            for row in rows:
                row.checkbox.Checked += self._on_export3d_selection_changed
                row.checkbox.Unchecked += self._on_export3d_selection_changed

        self._update_export3d_estimate()

    def _on_export3d_selection_changed(self, sender, e):
        """
        Does: reacts to a checkbox toggle in the "Export 3D" tab.
        Depends on: self._update_export3d_estimate.
        Returns: nothing (side effect: refreshes lblExport3DEstimate).
        """
        self._update_export3d_estimate()

    def _update_export3d_estimate(self):
        """
        Does: refreshes the "Export 3D" tab's duration estimate from the number of checked rows,
        using the rough EXPORT_3D_SECONDS_PER_ITEM/view figure (00_constants.py).
        Depends on: self._export3d_sections, EXPORT_3D_SECONDS_PER_ITEM (00_constants.py), self.lbl_export3d_estimate.
        Returns: nothing (side effect on self.lbl_export3d_estimate.Text).
        """
        selected_count = 0
        for section in self._export3d_sections.values():
            for row in section["rows"]:
                if row.checkbox.IsChecked:
                    selected_count += 1

        if selected_count == 0:
            self.lbl_export3d_estimate.Text = "No 3D view selected."
            return

        total_seconds = selected_count * EXPORT_3D_SECONDS_PER_ITEM
        minutes = total_seconds // 60
        seconds = total_seconds % 60
        if minutes:
            duration_text = "{} min {:02d} s".format(minutes, seconds)
        else:
            duration_text = "{} s".format(seconds)

        view_word = "view" if selected_count == 1 else "views"
        self.lbl_export3d_estimate.Text = "{} {} selected - estimated time: ~{} ({} s/view)".format(
            selected_count, view_word, duration_text, EXPORT_3D_SECONDS_PER_ITEM)

    def _attach_list_fade(self, scroll, fade_height=ITEM_LIST_FADE_HEIGHT):
        """
        ...
        stretched, unlike the fixed height of the preview cards). fade_height is configurable so
        the configuration side panel can use a taller fade blending into its fixed Apply/Cancel
        footer (see CONFIG_PANEL_FOOTER_FADE_HEIGHT) instead of the row lists' default.
        ...
        if content is not None:
            content.Margin = Thickness(0, 0, 0, fade_height)

        def update_fade(sender, e):
            """
            Does: recalculates scroll's opacity mask on every scroll/content change.
            Depends on: scroll/fade_height (captured by the closure).
            ...
            if height <= 0 or scroll.ScrollableHeight <= 0:
                ...
                return
            fade_start = 1.0 - min(fade_height / height, 0.5)
    ...
    def _open_config_panel(self, kind, row_config, refresh_callback, bulk_rows=None):
        ...
        panel.Children.Clear()
        self.panel_config_panel_footer.Children.Clear()
        ...
        # Apply/Cancel go in the fixed footer (panelConfigPanelFooter, overlaid on top of
        # scrollConfigPanel - see the XAML), not in panel itself: they must stay reachable
        # without scrolling even on a long configuration (e.g. "Results").
        footer = self.panel_config_panel_footer

        btn_apply = Button()
        btn_apply.Style = _shared_resources["SuccessButtonStrong"]
        ...
        btn_cancel = Button()
        btn_cancel.Style = _shared_resources["DangerButtonStrong"]
    ...
    def _show_multi_result_picker(self, index, current_result=None):
        """
        Does: shows in the side panel the (searchable) list of results available for cell index,
        grouped under 3 category headers (Results, Bolt Tool, Contact Tool Results - same
        categories/order as the "Result categories" tab).
        Depends on: self.panel_multi_result_side, self._results/_bolt_tool_results/_contact_tool_results,
            self._init_search_placeholder, self._make_multi_result_pick_handler.
        Returns: nothing (side effect on panelMultiResultSidePanel; repopulates self._mr_picker_entries/_mr_picker_list_panel).
        """
        self._mr_editing = None
        self.panel_multi_result_side.Children.Clear()
        ...
        search_box.ToolTip = "Enter = bring every matching row (or a whole matching category) to the top of the list"
        self._init_search_placeholder(search_box)
        search_box.KeyDown += self._on_multi_result_picker_search_key_down
        self.panel_multi_result_side.Children.Add(search_box)

        list_panel = StackPanel()
        self._mr_picker_list_panel = list_panel
        scroll = ScrollViewer()
        ...
        self._mr_picker_entries = []
        # Same 3 categories, same order, as the "Result categories" tab (03): Results, then
        # Bolt Tool, then Contact Tool Results - so a single result can be picked for a
        # combined-slide cell regardless of which category it comes from.
        picker_categories = [
            ("Results", self._results),
            ("Bolt Tool", self._bolt_tool_results),
            ("Contact Tool Results", self._contact_tool_results),
        ]
        for category_name, tagged_objects in picker_categories:
            objects = [obj for obj, _analysis in tagged_objects]
            if not objects:
                continue

            header_border = Border()
            header_border.Background = _shared_resources["SecondaryBackgroundBrush"]
            header_border.Padding = Thickness(6, 4, 6, 4)
            header_border.Margin = Thickness(0, 8, 0, 4)
            header_text = TextBlock()
            header_text.Text = category_name
            header_text.FontWeight = FontWeights.Bold
            header_text.FontSize = 11
            header_border.Child = header_text
            list_panel.Children.Add(header_border)
            self._mr_picker_entries.append({"kind": "header", "border": header_border,
            ...
            for result in objects:
                ...
                self._mr_picker_entries.append({"kind": "row", "border": row_border,
                                                 "text_block": row_text, "result": result,
                                                 "category": category_name})

    def _on_multi_result_picker_search_key_down(self, sender, e):
        """
        Does: triggers the "Choose a result" search when the user presses Enter.
        Depends on: self._perform_multi_result_picker_search.
        Returns: nothing (side effect: sets e.Handled and launches the search).
        """
        if e.Key == Key.Enter:
            e.Handled = True
            self._perform_multi_result_picker_search(sender)

    def _perform_multi_result_picker_search(self, search_box):
        """
        Does: brings every matching entry of the "Choose a result" list to the top, as a block -
        a row matches if its own name OR its category's name contains the typed text (so
        searching a category, e.g. "Bolt Tool", brings that whole category to the top even if no
        individual result name matches), a header matches if its category name contains it. Same
        "classic" reorder-to-top behavior as the other selection lists (see
        ReportGeneratorApp._perform_search), adapted to this picker's own (header/row) data model
        (rows are plain Border/TextBlock click targets here, not SectionRow/checkbox).
        Depends on: search_box.Text, self._mr_picker_entries/_mr_picker_list_panel,
            self._reset_multi_result_picker_entry_border, SEARCH_HIGHLIGHT_BRUSH, SEARCH_BOX_*_BACKGROUND.
        Returns: nothing (side effect: reorders self._mr_picker_list_panel.Children and colors the
            borders/field; an empty search field restores the original category order).
        """
        ...
        list_panel = self._mr_picker_list_panel

        if not query:
            for entry in self._mr_picker_entries:
                self._reset_multi_result_picker_entry_border(entry)
            list_panel.Children.Clear()
            for entry in self._mr_picker_entries:
                list_panel.Children.Add(entry["border"])
        ...
        others = []
        for entry in self._mr_picker_entries:
            category_matches = query in entry["category"].lower()
            if entry["kind"] == "header":
                is_match = category_matches
            else:
                is_match = category_matches or query in entry["text_block"].Text.lower()
            (matches if is_match else others).append(entry)

        for entry in matches:
            entry["border"].BorderThickness = Thickness(2)
            entry["border"].BorderBrush = SEARCH_HIGHLIGHT_BRUSH
        for entry in others:
            self._reset_multi_result_picker_entry_border(entry)

        list_panel.Children.Clear()
        for entry in matches + others:
            list_panel.Children.Add(entry["border"])

        if matches:
            matches[0]["border"].BringIntoView()
            search_box.Background = SEARCH_BOX_DEFAULT_BACKGROUND
        else:
            search_box.Background = SEARCH_BOX_NO_MATCH_BACKGROUND

    def _reset_multi_result_picker_entry_border(self, entry):
        """
        Does: restores a "Choose a result" entry's border to its default (non search-highlighted) style.
        Depends on: entry["kind"]/["border"], _shared_resources["CardBorderBrush"].
        Returns: nothing (side effect on entry["border"]).
        """
        if entry["kind"] == "header":
            entry["border"].BorderThickness = Thickness(0)
        else:
            entry["border"].BorderThickness = Thickness(0, 0, 0, 1)
            entry["border"].BorderBrush = _shared_resources["CardBorderBrush"]
    ...
    def _perform_search(self, search_box, rows, panel):
        """
        Does: brings every row of rows whose name contains the typed text to the top of panel,
        as a single block (each group - matches then non-matches - keeps rows' original relative
        order), and outlines the matching rows in blue. Purely a visual reorder/highlight: unlike
        the previous "find next" behavior, it no longer checks the matching row's checkbox
        (searching must never change the report selection).
        Depends on: search_box.Text, rows, panel, SEARCH_HIGHLIGHT_BRUSH, SEARCH_BOX_*_BACKGROUND.
        Returns: nothing (side effect: reorders panel.Children and colors the borders/field; an
            empty search field restores rows' original order).
        """
        text = search_box.Text
        if text == SEARCH_PLACEHOLDER:
            text = ""
        ...
        if not query:
            for row in rows:
                row.border.BorderThickness = Thickness(0)
            panel.Children.Clear()
            for row in rows:
                panel.Children.Add(row.border)
            search_box.Background = SEARCH_BOX_DEFAULT_BACKGROUND
        ...
        matches = [row for row in rows if query in row.text_block.Text.lower()]
        matched_set = set(matches)
        others = [row for row in rows if row not in matched_set]
        ...
        # Matches first (in their original relative order), then every other row, so the block
        # of matches sits together at the top without disturbing the rest of the list order.
        panel.Children.Clear()
        for row in matches + others:
            panel.Children.Add(row.border)
    ...
    def _wire_export3d_zone_buttons(self):
        """
        Does: wires the "Select all"/"Deselect all" buttons of each "Export 3D" tab zone (XAML card header).
        Depends on: self._export3d_sections, self.window.FindName, self._make_export3d_zone_toggle_handler.
        Returns: nothing (side effect: wires the Click of the btnZoneCheck{name}/btnZoneUncheck{name} buttons).
        """
        for name in self._export3d_sections:
            check_btn = self.window.FindName("btnZoneCheck" + name)
            uncheck_btn = self.window.FindName("btnZoneUncheck" + name)
            if check_btn is not None:
                check_btn.Click += self._make_export3d_zone_toggle_handler(name, True)
            if uncheck_btn is not None:
                uncheck_btn.Click += self._make_export3d_zone_toggle_handler(name, False)

    def _set_export3d_section_checked(self, name, checked):
        """
        Does: (un)checks every row of a single "Export 3D" tab zone.
        Depends on: self._export3d_sections[name]["rows"].
        Returns: nothing (side effect on the zone's CheckBoxes).
        """
        for row in self._export3d_sections[name]["rows"]:
            row.checkbox.IsChecked = checked

    def _make_export3d_zone_toggle_handler(self, name, checked):
        """
        Does: closes over name/checked to produce the Click handler for an "Export 3D" zone's "Select all"/"Deselect all" button.
        Depends on: self._set_export3d_section_checked.
        Returns: function, the handler(sender, e) to wire to the zone's button.
        """
        def handler(sender, e):
            self._set_export3d_section_checked(name, checked)
        return handler

    def _on_check_all_export3d(self, sender, e):
        """
        Does: checks every row of the "Export 3D" tab (all 3 zones).
        Depends on: self._export3d_sections, self._set_export3d_section_checked.
        Returns: nothing (side effect on the tab's CheckBoxes).
        """
        for name in self._export3d_sections:
            self._set_export3d_section_checked(name, True)

    def _on_uncheck_all_export3d(self, sender, e):
        """
        Does: unchecks every row of the "Export 3D" tab (all 3 zones).
        Depends on: self._export3d_sections, self._set_export3d_section_checked.
        Returns: nothing (side effect on the tab's CheckBoxes).
        """
        for name in self._export3d_sections:
            self._set_export3d_section_checked(name, False)
    ...
    def _on_export_selected_3d(self, sender, e):
        """
        Does: exports to .avz only the rows checked in the "Export 3D" tab, into EXPORT_3D_FOLDER
        (same folder used by the original "Export to 3D (.avz)" toolbar button).
        Depends on: self._export3d_sections, export_object_3d_view/ensure_folder_exists (05_interactive_slides.py/00_constants.py),
            EXPORT_3D_FOLDER (00_constants.py), _print_console_banner.
        Returns: nothing (side effect: creates .avz files in EXPORT_3D_FOLDER, shows a MessageBox
            if nothing is selected or on failure).
        """
        selected_objects = []
        for section in self._export3d_sections.values():
            for row in section["rows"]:
                if row.checkbox.IsChecked:
                    selected_objects.append(row.row_config.obj)

        if not selected_objects:
            MessageBox.Show("No 3D view selected.", "Export 3D",
        ...
        try:
            ...
            for obj in selected_objects:
                if export_object_3d_view(obj, EXPORT_3D_FOLDER):
                    exported_count += 1
            if exported_count:
    ...
    def _get_or_create_report(self):
        """
        Does: reuses the PowerPoint working copy already open from a previous "Generate report"
        click in this window session (self._current_report), so slides accumulate in the SAME
        file instead of opening a new copy every time. Falls back to opening a fresh copy if no
        report is open yet, or if the previous one is no longer reachable (e.g. the user closed
        the PowerPoint window by hand between two clicks) - detected via a harmless COM call,
        never surfaced as an error, just a console note.
        Depends on: self._current_report, PPTReportBuilder, TEMPLATE_PATH.
        Returns: tuple (PPTReportBuilder, bool) - the report to add slides to, and whether it is a
        freshly opened working copy for this call (True) or a reused one (False). The caller
        (_on_generate) uses this to decide whether it is safe to close/discard the copy on error.
        """
        if self._current_report is not None:
            try:
                self._current_report.presentation.Slides.Count
                print "Adding to the report already open: " + self._current_report.working_copy_path
                return self._current_report, False
            except Exception as ex:
                print "Previous report no longer reachable ({}): opening a new copy.".format(str(ex))
                self._current_report = None

        print "Opening PowerPoint template..."
        self._current_report = PPTReportBuilder(TEMPLATE_PATH)
        return self._current_report, True

    def _reset_all_selections(self):
        """
        Does: unchecks every selection across tabs 01-04 (General/Conditions/Results/Combined
        slide) after a successful "Generate report" - called only on success, so the just-added
        slides are never silently re-added on the next click. The "Export 3D" tab is deliberately
        left untouched: its selection is unrelated to the report (see self._export3d_sections).
        Depends on: self.chk_geometry/chk_mesh, self._set_group_checked, self._multi_result_slides, self._update_preview.
        Returns: nothing (side effect: unchecks every row/checkbox and clears the report preview).
        """
        self.chk_geometry.IsChecked = False
        self.chk_mesh.IsChecked = False
        self._set_group_checked("general", False)
        self._set_group_checked("conditions", False)
        self._set_group_checked("results", False)
        self._multi_result_slides = []
        self._update_preview()

    def _on_generate(self, sender, e):
        """
        Does: generates the PowerPoint report following the order of self._preview_order, appending
        to the working copy already open from a previous click in this window session if there is
        one (see self._get_or_create_report), instead of opening a new copy every time.
        Depends on: self._get_or_create_report, build_*_slides/create_*_slide (04_slides.py/05_interactive_slides.py),
            apply_view_if_exists/self._geometry_view_config/_mesh_view_config, self._update_generation_progress,
            self._reset_all_selections.
        Returns: nothing (side effect: creates/appends to the PPTX file, updates the status UI,
            resets the tabs' selections on success, shows a MessageBox on failure).
        """
        ...
        try:
            ...
            report, is_new_report = self._get_or_create_report()
            ...
            for index, (kind, payload) in enumerate(self._preview_order):
                if kind == "general":
                    if payload == "Geometry":
                        ...
                        create_geometry_slide(report, self._geometry_view_config.image_format)
                    elif payload == "Mesh":
                        apply_view_if_exists(self._mesh_view_config.view_name, self._views)
                        build_mesh_slide(report, self._mesh_table_full, self._mesh_view_config.image_format)
            ...
            # Clears every checkbox (Export 3D tab excluded, unrelated to the report) so the next
            # "Generate report" click only adds a NEW batch of slides to this same file, instead
            # of silently re-adding what was just generated.
            self._reset_all_selections()
        except Exception as ex:
            _print_console_banner("ERROR DURING REPORT GENERATION")
            print str(ex)
            # Only closes/discards a working copy that was JUST opened for this call: a REUSED
            # report (appending to a previous successful generation) is left open as-is - it may
            # already contain valid slides from earlier batches that must not be thrown away
            # because this new batch failed.
            if report is not None and is_new_report:
                try:
                    report.close()
    ...
    def _wire_events(self):
        ...
        self.btn_check_all_export3d.Click += self._on_check_all_export3d
        self.btn_uncheck_all_export3d.Click += self._on_uncheck_all_export3d
        self.btn_export_selected_3d.Click += self._on_export_selected_3d
...
# No callback, no manifest: running this file IS the entry point. Load it in Mechanical's
# scripting console (Automation tab) and click "Run script" to open the report generation
# window directly. Once confirmed working, use "Show Button Editor" > "Promote script to button"
# (last button of the scripting console toolbar) to turn it into a persistent toolbar button for
# this project - see README.md, "Manual installation (scripting console)".
