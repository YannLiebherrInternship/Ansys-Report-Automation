...
# === Root paths, computed from PROJECT_DIR (already defined by AnsysReportGenerator_WPF.py before the execfile() of this file, = the "Report Generator" folder created by hand next to this project's user_files) ===
# No path is hardcoded for a specific machine/project: PROJECT_DIR is located via
# ExtAPI.DataModel.Project.ProjectDirectory (see _prepare_environment in
# AnsysReportGenerator_WPF.py), i.e. one copy of this "Report Generator" folder per Ansys project
# (see README.md, "Manual installation"). DATA_ROOT and the folders below therefore live next to
# this script, and are created automatically on first run if they don't already exist (see the
# ensure_folder_exists() calls at the bottom of this file).
...
# Rough estimate (measured empirically, varies with model size/graphics card) used only to
# display a duration estimate in the "Export 3D" tab (see ReportGeneratorApp._update_export3d_estimate
# in AnsysReportGenerator_WPF.py) - not a guaranteed value.
EXPORT_3D_SECONDS_PER_ITEM = 25

    # Legends live in this project's own "user_files/legend" folder (standard Ansys project
    # folder, same base as PROJECT_DIR itself in this manual/console flavor - see
    # _prepare_environment in AnsysReportGenerator_WPF.py) rather than under DATA_ROOT - this
    # script only READS them, never generates them. Falls back to a local data/legend folder
    # (with a console warning) if the project was never saved and ProjectDirectory is empty, so a
    # missing/unsaved project degrades gracefully instead of crashing the whole script.
try:
...
# === Image export format: 4 mutually exclusive formats, chosen per row via the "Image format"
# radio buttons in the settings panel (see _build_row_config_fields/_build_geometry_part_fields/
# _build_mesh_part_fields in AnsysReportGenerator_WPF.py). IMAGE_FORMAT_DIMENSIONS maps each
# format constant to the (width, height) passed to GraphicsImageExportSettings. The "_HQ" formats
# are the same shape at double resolution (3840 instead of 1920).
IMAGE_FORMAT_SQUARE = "square"
IMAGE_FORMAT_RECTANGULAR = "rectangular"
IMAGE_FORMAT_SQUARE_HQ = "square_hq"
IMAGE_FORMAT_RECTANGULAR_HQ = "rectangular_hq"
DEFAULT_IMAGE_FORMAT = IMAGE_FORMAT_SQUARE
IMAGE_FORMAT_DIMENSIONS = {
    IMAGE_FORMAT_SQUARE: (1920, 1920),
    IMAGE_FORMAT_RECTANGULAR: (1920, 1080),
    IMAGE_FORMAT_SQUARE_HQ: (3840, 3840),
    IMAGE_FORMAT_RECTANGULAR_HQ: (3840, 2160),
}
