...
def export_current_view_image(image_name, image_format=None):
    """
    Does: exports the current graphics view to PNG (Ansys logo and ruler hidden) with the report's default settings.
    Depends on: ExtAPI.Graphics.ExportImage/ViewOptions (Ansys API), get_unique_file_path/IMAGE_FORMAT_DIMENSIONS/DEFAULT_IMAGE_FORMAT (00_constants.py).
    Returns: str, the path of the written PNG file.
    """
    # ShowLogo/ShowRuler forced here (common pass-through point for all image exports) rather than
    # in each high-level export function: guarantees that no report image shows
    # the Ansys logo or the ruler, without having to remember it in every new caller.
    ExtAPI.Graphics.ViewOptions.ShowLogo = False
    ExtAPI.Graphics.ViewOptions.ShowRuler = False
    ...
    width, height = IMAGE_FORMAT_DIMENSIONS.get(image_format, IMAGE_FORMAT_DIMENSIONS[DEFAULT_IMAGE_FORMAT])
...
def export_geometry_image(image_format=None):
    """
    ...
    return export_current_view_image("geometry", image_format)


def export_mesh_image(image_format=None):
    """
    ...
    return export_current_view_image("mesh", image_format)


def export_analysis_overview_image(analysis=None, image_format=None):
    """
    ...
    analysis.Activate()
    return export_current_view_image(analysis.Name, image_format)


def export_object_image(obj, image_name, image_format=None):
    """
    ...
    obj.Activate()
    return export_current_view_image(image_name, image_format)


def export_solution_image(result, image_format=None):
    """
    Does: exports the view of a given solution result.
    Depends on: export_object_image.
    Returns: str, the path of the PNG.
    """
    return export_object_image(result, result.Name, image_format)
