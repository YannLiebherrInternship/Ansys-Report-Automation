...
class ReportGeneratorApp(object):
    ...
    def __init__(self, xaml_path):
        ...
        self._current_report = None    # PPTReportBuilder kept open across "Generate report" clicks (appends instead of reopening), see _get_or_create_report/_on_new_report
    ...
    def _find_controls(self):
        ...
        self.btn_new_report = w.FindName("btnNewReport")
    ...
    def _on_new_report(self, sender, e):
        """
        Does: "New report" button - discards the reference to the PowerPoint working copy
        currently open, WITHOUT closing PowerPoint itself (left open for the user to review/save
        or discard by hand, same as at the end of a normal generation) - so the next "Generate
        report" click opens a brand-new copy of the template instead of appending to this one.
        Depends on: self._current_report.
        Returns: nothing (side effect: self._current_report reset to None, console message).
        """
        if self._current_report is None:
            print "No report currently open."
            return
        print "Report session ended (file left open in PowerPoint, not closed): " + self._current_report.working_copy_path
        self._current_report = None
    ...
    def _wire_events(self):
        ...
        self.btn_new_report.Click += self._on_new_report
