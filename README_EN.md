...
Three methods are available. In all three cases the resulting interface, once the application is open (see §6), is strictly identical — only the deployment mode, the name under which the tool shows up, and the launch mode change.

| | Method 1 — `.wbex` extension | Method 2 — Manual ACT extension | Method 3 — Manual button (console) |
|---|---|---|---|
| Source folder (this repo) | `Reports/Liebherr Report Generator.xml` + `Reports/Liebherr Report Generator/` | Same as Method 1 | `Report Generator/` (separate folder, at the repo root) |
| Installation | Once (Extension Manager), valid for every project | Once ("extension" folder + Options), valid for every project | Manual folder copy, project by project |
| File paths | Handled automatically, the extension detects the host Workbench project itself | Same as Method 1 | Computed on first launch from the `Report Generator` folder created by hand in the project |
| Access | A dedicated tab in the ACT ribbon, built into the extension | Same as Method 1 | Button "promoted" into the Automation tab, persistent across sessions of the same project |
| File cleanup | The extension's "Files" tab — manual, final check required | Same as Method 1 | The application's "Files" tab (see §6) |
| Uninstallation | Automatic: the extension deletes every file associated with it | Uncheck/remove the extension in the Extension Manager | Manual deletion of the `Report Generator` folder |

### 2.1 Method 1 — Ansys extension (`.wbex`)
...
### 2.2 Method 2 — Manually installed ACT extension (no `.wbex`)

Installs the same ACT extension as Method 1, without going through an already-built `.wbex` file — starting directly from the repository's source files instead. Useful to test a local code change before publishing it as a `.wbex`, or when no `.wbex` file is available.

1. Locate `Reports/Liebherr Report Generator.xml` and the `Reports/Liebherr Report Generator/` folder of the same name in this repository (the manifest and its payload — see §3).
2. Create a folder named exactly `extension` on the machine (tested location: next to a `user_files` folder of an Ansys project — the exact location doesn't matter much, as long as it's a stable folder on the machine, since it is registered once as an additional extension folder in the next step).
3. In Ansys Workbench: **Tools** → **Options** → **Extensions**, then paste the full path of the `extension` folder created in the previous step (without quotes).
4. Go into that `extension` folder and drop the `Liebherr Report Generator/` folder **and** the `Liebherr Report Generator.xml` file of the same name into it (both, side by side, exactly as in this repository).
5. Still in Workbench, open the **Extension Manager**, check the "Liebherr Report Generator" extension, reload, then open Mechanical to use it — the tab appears just like in Method 1.
6. **Optional**: from this manual install, it is possible to build your own `.wbex` file (to redistribute it afterward as in Method 1) by going to the ACT start page (**ACT Start Page**) and clicking **"Build"** in the loaded extension's settings.

### 2.3 Method 3 — Manual button in the scripting console

Unlike the two previous methods, **this is not an ACT extension**: no `.xml` file, no manifest, no `oninit`/`onclick` — the `AnsysReportGenerator_WPF.py` script runs directly from start to finish when loaded in the scripting console, exactly like any other Mechanical script. This is the simplest method to quickly test a code change without going through ACT.

> The `Report Generator/` folder at the root of this repository is the starting point for this method: it is deliberately kept **separate** from `Reports/Liebherr Report Generator/` (Methods 1 and 2, which are a real ACT extension). It holds a copy of the same application code, but without a manifest and without ACT's `init`/`HighFiveOut` entry point — the script runs directly on execution, the way this project's very first desktop script did before it was converted into an extension.
...
3. Copy flat into this folder, with no subfolder, the entire contents of this repository's `Report Generator/` folder:
...
This repository holds two distinct payloads, deliberately kept separate (see §2):

| Folder | Role |
|---|---|
| `Reports/Liebherr Report Generator.xml` + `Reports/Liebherr Report Generator/` | Full ACT extension (manifest + payload) — Methods 1 and 2 (§2.1, §2.2) |
| `Report Generator/` | Same application, without the ACT wrapping (no manifest, directly-executed script) — Method 3 (§2.3) |

The rest of this documentation (§4 onward) describes the application code shared by all three methods; the `Report Generator/` folder is what to deploy for Method 3 (§2.3).

Content of `Report Generator/` in detail (Method 3):
